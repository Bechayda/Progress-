...
public:
    ...
    // ✅ Check if a sound has stopped
    bool isSoundStopped(const std::string& name) const {
        auto it = sounds.find(name);
        if (it != sounds.end()) {
            return it->second.getStatus() == sf::Sound::Stopped;
        }
        return true; // If not found, treat as "not playing"
    }
