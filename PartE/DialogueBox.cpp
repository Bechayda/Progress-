...
DialogueBox::DialogueBox(sf::RenderWindow& win, sf::Font& fnt, SoundManager& sndMgr)
    : window(win), font(fnt), soundManager(sndMgr), currentCharIndex(0), visible(false),
    finishedTyping(false), typeSpeed(50.0f), hasBackgroundImage(false),
    autoForwardEnabled(false), backPressed(false), blockedSoundName("")
{
...
void DialogueBox::update() {
    ...
    if (!blockedSoundName.empty()) {
        if (soundManager.isSoundStopped(blockedSoundName)) {
            blockedSoundName.clear(); // Unblock input
        }
        else {
            return; // Wait until sound finishes
        }
    }

    bool soundFinished = blockedSoundName.empty();
    if (autoForwardEnabled && finishedTyping && soundFinished && advanceClock.getElapsedTime().asSeconds() >= 1.0f) {
        nextDialogue();
    }
    ...
    if (forwardButton.getGlobalBounds().contains(mousePos)) {
        forwardButton.setFillColor(sf::Color(130, 130, 255));
    }
    else {
    ...
    if (backButton.getGlobalBounds().contains(mousePos)) {
        backButton.setFillColor(sf::Color(255, 130, 130));
    }
    else {
...
void DialogueBox::handleInput(const sf::Event& event) {
    ...
    bool soundFinished = blockedSoundName.empty() || soundManager.isSoundStopped(blockedSoundName);
...
void DialogueBox::blockAdvanceUntil(const std::string& soundName) {
    blockedSoundName = soundName;
}
