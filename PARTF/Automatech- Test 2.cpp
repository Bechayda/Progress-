...
int main() {
    ...
    if (!soundManager.loadSound("bell", "bell.ogg")) {
        std::cerr << "Failed to load bell sound effect.\n";
    }
    ...
    while (window.isOpen()) {
        if (state == TitleScreen::GameState::TITLE) {
            ...
            DialogueBox dialogue(window, bodyFont, SoundManager::getInstance());
            ...
            if (loadedFromSave) {
                ...
                std::vector<DialogueEntry> dialogues = {
                    ...
                    {"Saan bahay ni Auntie?", "[Flashback]???"},
                ...
                dialogue.startDialogue(dialogues);
            }
            ...
            while (window.isOpen()) {
                ...
                if (dialogue.wasBackPressed()) {
                    dialogue.resetBackPressed();
                    showDialogue = false;  // Hide dialogue mode
                    state = TitleScreen::GameState::TITLE; // Go back to main menu
                }

                window.clear(sf::Color::Black);
                bgManager.draw();

                std::string currentDialogue = dialogue.getCurrentDialogue();
                ...
                if (showDialogue) {
                    ...
                    if (currentDialogue == "Apo, puntahan mo nga muna bahay nila Auntie Amelita mo. Bigyan mo sya nang \nnatirang maja." && !flashShown) {
                        bgManager.setBackground("Tess.png");
                        flashShown = true;
                    }

                    if (currentDialogue == "Sa tapat daw ng municipal hall...." && !bb) {
                        bgManager.setBackground("bgs.png");
                        bb = true;
                    }

                    if (currentDialogue == "Ang dilim." && !blackShown) {
                        bgManager.setBackground("BlackBG.jpg");  
                        blackShown = true;
                    }
                
                    if (currentDialogue == "Rub eyes." && !rubShown) {
                        bgManager.setBackground("Rubs.png");
                        rubShown = true;
                    }

                    if (currentDialogue == "Fuck" && !bg12) {
                        bgManager.setBackground("Back.png");
                        bg12 = true;
                    }
                    ...
                    if (currentDialogue == "Looks back") {
                       ...
                       // dialogue.blockAdvanceUntil("ECH1");
                    }
                    ...
                    if (currentDialogue == "Look Back.\nPan on the city of Biringan" && !chapter2Started) {
                        ...
                        std::vector<DialogueEntry> chapter2Dialogues = {
                            ...
                            {"Kada tunog ng kampana ay padagdag ng padagdag ang takot sa puso ko. Pinwersa\nko ang sarili ko na tignan ang nangyayari sa likod.", "???"},
                            {"Nakita kong binubuksan ang pinto ng isang lumang gusali.", "???"},
                            {"Namangha ako sa Nakita kong display.", "???"},
                            {"Marangya lang ang mad-describe ko sa nakita ko. Gintong mga karwahe, mga\nmamahaling kotse,", "???"},
                            //Background image 
                        ...
                        dialogue.startDialogue(chapter2Dialogues);
                    }

                    if (currentDialogue == "Napahakbang ako pabalik ng marinig ko ang tunog ng kampana. Nagsitahimikan\nang mga tao.") {
                        soundManager.playSound("bell");
                        // dialogue.blockAdvanceUntil("ECH1");
                    }

                    if (currentDialogue == "Nakita kong binubuksan ang pinto ng isang lumang gusali." && !bg12) {
                        bgManager.setBackground("Back.png");
                        bg12 = true;
                    }

                    if (currentDialogue == "Namangha ako sa Nakita kong display." && !buk) {
                        bgManager.setBackground("Bukas.png");
                        buk = true;
                    }

                    if (currentDialogue == "Marangya lang ang mad-describe ko sa nakita ko. Gintong mga karwahe, mga\nmamahaling kotse," && !ung) {
                        bgManager.setBackground("Ung.png");
                        ung = true;
                    }

                    if (currentDialogue == "Tawagan mo nga si Princess, kanina pa tayo nagi-intay." && !birig2) {
                        bgManager.setBackground("Biringan2.jpg");
                        birig2 = true;
                    }
                    ...
                    if (currentDialogue == "Pumunta ba ako sa isa o bumalik ulit dun at i-try bumalik?" && !choiceBox.isVisible()) {
                        std::vector<Choice> choices = {
                            {"Pumunta", [&]() {
                                ...
                                dialogue.startDialogue(choice1Dialogue);
                                showDialogue = true; // Fix works now
                            }},
                            {"Bumalik", [&]() {
                                std::vector<DialogueEntry> choice2Dialogue = {
                                    {"Naisipan kong bumalik ulit. Mas malaking chance na makabalik ako at nandun pa\nyung nailaw na puno. ", "???"}, //SOUND EFFECTS OF WALKING
                                    {"Pagkabalik ko, mas konti na yung tao na nagdiriwang pero masigla parin ang mga\ntao. Huminto ako sa harap ng puno. Hindi na sya nakinang katulad nung una at parang\nordinaryo lang sya. Matagal ko siyang tinitigan at sinayayat pati na rin ang malapit\nsa paligid niya pero wala talaga akong makitang way para makauwi.", "???"},
                                    {"Tumingin-tingin ulit ako sa paligid at napansin ko ang bukas na gusali sa harap. Sa\npasukan pa lang ay kita na ang ganda ng kalikasan. Sure ako na marami ang puno at\nmga magagandang bulaklak.", "???"},
                                    {"Napakaganda siguro pumasok at magpahinga dyan.", "???"},
                                    {"[Bakit naman hindi? Minsan ka lang naman makapunta dito]", "???"},
                                    {"Kailangan ko makauwi…", "???"},
                                    {"[Sandali lang, titingin ka lang, Ano bang mali kung titingin ka lang?]", "???"},
                                    {"Hindi pede kailangan umuwi", "???"},
                                    {"[Maaga pa ang gabi, isang tingin lang, tapos pede kana ulit bumalik sa paghahanap]", "???"},
                                    {"Sige, isang tingin lang.", "???"},
                                    {"Pumunta ako sa harap ng gate at dun ko nakita nang mas maayos ang nasa loob nang\nGusali. Isang magandang kagubatan. Maraming mga puno at halaman at mga kutitap.", "???"},
                                    {"Gusto kong pumasok at hawakan at humiga sa damo. ", "???"},
                                    {"[Pumasok ka lang, hindi ka naman magtatagal]", "???"},
                                    {"Sandali lang naman ako.", "???"},
                                    {"Pumasok ako sa loob at nabighani ako sa mga nakikita ko. Matagal akong naglakad\nlakad hanggang sa may nakita akong ilog. Na-curious ako kung ano itsura sa\nmalapitan nito at kaya lumapit ako. ", "???"},
                                    {"Wala pang ilang hakbang, napansin ko na hindi lang ako magisa. May isa na nakaupo\nmalapit sa ilog at napapaligiran ng mga alitap tap.", "???"}, //PICTURE NI AELE WILL APPEAR HEAR
                                    {"Para akong nakakita ng isang diwato at parang bawal siyang istorbohin kung ayaw\nko masira ang nakikita ko. ", "???"},
                                    {"Pero mukhang nasira ko parin kasi napansin niya ang presensya ko. ", "???"},
                                    {"Maayong adlaw.", "Stranger"},
                                    {"Ma-maayong adlaww.", "???"},
                                    {"Halatang halata ang pagkanerbyos ko. ", "???"},
                                    {"Marikit ang gabing ito, Dumuloong. Mangyaring maupo ka muna at magpahinga—tila\nba matagal at mabigat ang iyong paglalakbay.", "Stranger"},
                                    {"At ba’t ako tatangi? Minsan lang ang opportunity na toh.", "???"},
                                    {"Umupo ako malapit sa kanya at nitatry magpokus sa tanawin. Siya na ma’y\nnagsimulang kumanta.", "???"}, //Sound Effect of singing voice
                                    {"Napaka-ethrerial nang kanyang boses. Para akong hinihimbing na matulog.", "???"},
                                    {"Pagkatapos niya kumanta ay tumingin sya sa may ilog.", "???"},
                                    {"Nasa kabilang baryo pa ang kasayahan… ngunit heto ka ngayon sa aking payak\nna tahanan. Anong banayad na ihip ng kapalaran ang naghatid sa iyo rito?", "???"},
                                    {"Sorry po! Hindi ko po sinasadya. Hindi ko po alam na sainyo–", "???"},
                                    {"Hindi ko pa natapos ang sinasabi ko nang napatawa sya.", "???"},
                                    {"Pahupain mo ang iyong kaba, palangga. Wala akong layuning ika’y saktan--\nako’y nagsasaya lamang.", "Stranger"},
                                    {"Paano mo nalaman pangalan ko?", "???"},
                                    {"Paanong ako'y magiging karapat-dapat na tagapag-alaga ng tahanang ito,\nkung ni hindi ko kilala ang mga nilalang na tumatawid sa kanyang hangganan?", "Stranger"},
                                    {"Wala akong maisip na gamitin na argumento. ", "???"},
                                    {"Hmm…", "???"},
                                    {"Maganda ang tahanan na napili ko di ba?", "Stranger"},
                                    {"Opo.", "???"},
                                    {"Gusto mo ba makita ang iba pa?", "Stranger"}
                                };
                                dialogue.startDialogue(choice2Dialogue);
                                showDialogue = true; // Fix works now
                            }}
                        };
                    ...
                    if (currentDialogue == "Kalahating oras pa ang nakalipas hanggang sa nakapunta na ako sa lugar ng isang\nlagusan. Tago sa loob ng isang malaking market." && !shop1) {
                        bgManager.setBackground("Shop1.png");
                        shop1 = true;
                    }

                    if (currentDialogue == "Sa kaliwa ako dumaan at napahanga ako sa mga bilihan. Para akong nasilaw ng\npagpasok ko. Ibang iba sya sa loob. Sari-sari ang binebenta at napakaraming pagkain,\nmga damit, laruan at kung ano-ano pa na binebenta sa mga ganitong market ang\nnandito. Parang bumalik lang ako sa Salcedo Market. Malalaki ng hinahaing pagkain\nat ang market ay hindi katulad sa labas na kulay black ang mga gusali." && !shop3) {
                        bgManager.setBackground("Shop3.png");
                        shop3 = true;
                    }
                    if (currentDialogue == "Napatingin ako sa mga binebentang pagkain. Chicken, Lobster, Seafoods, Lechon,\nCake at andami pa. Maraming puti at black na pagkain ditong binebenta. May kanin at\nparang pinipig na binebenta seperately. Di ko alam pero isa yan sa mga narinig kong\npagkain papunta dito, Tapul ata ang isa tawag nila." && !shop2) {
                        bgManager.setBackground("Shop2.png");
                        shop2 = true;
                    }
                    ...
                    if (currentDialogue == "Gusto mo ba makita ang iba pa?" && !choiceBox.isVisible()) {
                        std::vector<Choice> choices = {
                            {"Sumama", [&]() {
                                std::vector<DialogueEntry> choice1Dialogue = {
                                  
                                    {"ITUTULOY...", "???"}  //maybe image of to be continued
                        ...
                        std::vector<Choice> choices = {
                            {"Makipagkamay", [&]() {
                                std::vector<DialogueEntry> choice1Dialogue = {
                                    ...
                                    {"Pumunta kayo ng mga pinsan nyo sa covered court para manood ng mga maglalaro.", "[Flashback]???"},
                                    {"Naghahanap kayo ng mauupuan nang napansin mo ang isang maglalaro.", "[FlashBack]???"}, 
                                    {"Sa lahat ng maglalaro, sa kanya na pupunta ang atensyon ng lahat. Pano ba naman?\nSa tindig at galawa pa lang, ibang-iba na ang aura nya. Isama mo pa ang tangkad at\nkagwapuhan nya. May something sa kanya na parang nangaakit at hindi mo matanggal\nang iyong mga mata.", "[Flashback]???"}, //PICTURE OF MACA and background
                                    {"'[Name]!' Tawag sayo ng pinsan mo. Nabaling ang tingin mo sa pinsan na nakahanap\nna ng upuan at ilang beses ka nang tinatawag. ", "[Flashback]???"},
                                    {"'Wait lang!' Pasigaw mong sabi. Mabilis kang pumunta sa pwesto at umupo katabi\nnila.", "[Flashback]???"},
                                    {"'Insan, sino yung lalaking may mahabang buhok?' Tinuro mo yung lalaki.", "[Flashback]???"},
                                    {"Sya ? Si Maca. Apo ni Aleng Lina.Bumibisita rin katulad mo. Mga– Ilang taon ka na ba? ", "[Flashback]Pinsan"},
                                    {"19 po!", "[Flashback]???"},
                                    {"Kasing age mo lang. Mayayaman yang mga yan. Rinig ko nasa bandang manila yan\nnag-aaral. Di ko maalala kung anong school.", "[Flashback]Pinsan"},
                                    {"Ganun ba", "[Flashback]???"},
                                    {"Mhmm-mm", "[Flashback]???"},
                                    {"Marami pang nagsipasukan sa court, at napansin yun ni Mica. Lumapit sya sa iba sa\nkanila. Mukhang manonood yung pamilya nya sa mga palaro.", "[Flashback]???"},
                                    ///VESPER NIGHT
                                    {"Pinagayos at pinapunta kami ni Lola sa isa mga sa okasyon gabi bago mag-fiesta.\nVesper Night Socials yung pangalan ng event at gusto ni Lola na makisosyo sa mga\nkasing edad namin. ", "[Flashback]???"},
                                    {"Nainis pa si Lola nun at kalahating oras na ang nakalipas nang nag-start yung event\nat hindi pa kami tapos. ", "[Flashback]???"},
                                    {"Nang nakarating na kami sa event, iilan lang sa mga pinsan ko ang nakisosyo at yung\nmga nakipagusap lang ay yong nakatira na talaga dito habang yung iba kong pinsan\nna dumayo rin galing sa ibang lugar ay nakaupo at busy sa mga phone nila.", "[Flashback]???"},
                                    {"'Ano ba kayo dyan, puro selpon na lang inaatupag nyo!' Inis na sabi ni Lola.", "[Flashback]???"},
                                    {"Nasa party tayo tas nakayuko lang kayo dyan!", "[Flashback]Lola"},
                                    {"[Name], Yasmin, maki-sosyo kayo. Sumama kayo kay [Pinsan 1]. Maraming kakilala\nyan na kasing edad nyo!", "[Flashback]Lola"},
                                    {"Opo La.", "[Flashback]???"},
                                    {"Napatayo at napalapit na lang kami sa isa namin pinsan kesa suwayin pa si Lola.\nNang napansin na lang kami ng pinsan namin na palapit sa kanya, natawa na lang sya. ", "[Flashback]???"},
                                    {"'Pagpasensyahan nyo na si Lola,' patawa nyang sabi.", "[Flashback]Yasmin"},
                                    {"Ganyan talaga sya, bossy.", "[Flashback]Maki"},
                                    {"'Mga kaibigan ko nga pala.' Habang tinuturo at pinapakilala nya ang mga kaibigan.", "[Flashback]Yasmin"},
                                    {"Ngumiti na lang ako at kumaway.", "[Flashback]???"},
                                    {"Nagtuloy-tuloy lang ang usapan nila at mukhang nage-enjoy si Yasmin at nakikisama\nsya sa mga talakayan nila.", "[Flashback]???"},
                                    {"May upuan dun na tago medyo malapit sa buffet area, di ka gano mapapansin ni Lola.\nBanggit nya sakin.", "[Flashback]Yasmin"},
                                    {"Sige, dun muna ko Insan.", "[Flashback]???"},
                                    {"Sabi ko sa kanya at tinapik nya lang balikat bago siya nagfocus ulit sa pinag uusapan\nnila. ", "[Flashback]???"},
                                    {"Umaalis na ko at balak ko sanang dumiretso na dun sa upuan para maglaro pero\nnaramdaman ko ang gutom kaya dinaanan ko muna ang buffet area at kumuha ng\npaper plate and utensils. sa lang yung nagmamano sa table at busy sya na kuhaan\nyung mga bata na nakisama kaya ako na lang nagserve nang para sakin.", "[Flashback]???"},
                                    {"Tinitignan ko ng maigi yung pagkain, hindi ako pamilyar sa ibang putahe at di ko rin\nmatanong si kuyang nagmamano.", "[Flashback]???"},
                                    {"[Name ng putahe] yan.", "[Flashback]???"},
                                    {"Nagulat ako bigla sa nagsalita sa likod ko. Muntikan na mahulog yung kanin na nasa\npaper plate ko", "[Flashback]???"},
                                    {"Lumingon ako at napahanga ako sa nakita ko. Mas kulot ang buhok nya ngayon at\nnaka-suot sya ng kulay grey na dress shirt at nakatupi ang mga mangas nya at dalawa\nang di nakabutones. Sinamahan nya ito ng black na pants at kino-complement nito\nang figure niya. Tumingala ako para makita  siya ng maayos. ", "[Flashback]???"},
                                    {"Ano yun? Tanong ko", "[Flashback]???"},
                                    {"[Explanation kung ano yung food.] Sabi niya.", "[Flashback]???"},
                                    {"'Salamat.'  Sabi ko habang nagbow sa kaniya.", "[Flashback]???"},
                                    {"Mahina syang tumawa at parang natutuwa sya sa ginawa ko. ", "[Flashback]???"}, 
                                    {"Kanino kang Apo?", "[Flashback]Stranger"},
                                    {"Kay Lola Irina.", "[Flashback]???"},
                                    {"Ah.. ikaw yung apo nya na galing maynila.", "[Flashback]Stranger"},
                                    {"Oo, nabangit sakin ng pinsan ko na sa maynila ka rin naga-aral, tama ba?", "[Flashback]???"}, 
                                    {"Parang nagulat sya ng konti sa sinabi ko pero mabilis nyang inayos ang ekspresyon\nnya.", "[Flashback]???"},
                                    {"'Oo, Maca nga pala.' Sabi niya sabay nilahad nya yung kamay niya.", "[Flashback]Maca"},
                                    {"'[Name]' Sabi ko at nakipagkamay sa kanya. ", "[Flashback]???"},
                                    {"Sa buong gabing yun, sya lang yung kinausap ko. Isa yun sa mga masayang nangyari\nnung araw na yun. Di ko na sya nakita pa pagkatapos nung gabi yan. ", "[Flashback]???"}, //END OF FLASHBACK
                                    ...
                                    {"Hindi pero..", "???"},
                                    {"Maikli lang ang buhay ng tao kaya ginagawa silang engkanto para magkasama sila\nng habang buhay. Ang paraan para maging engkanto ay ang pagkuha ng kaluluwa nila\nat i-bind silang dalawa.", "Maca"},
                            ...
                            {"Tumitig", [&]() {
                                std::vector<DialogueEntry> choice2Dialogue = {
                                    ...
                                    {"Wala ka ring modo, noh?", "Male Stranger"},
                    ...
                    if (currentDialogue == "Pumunta kayo ng mga pinsan nyo sa covered court para manood ng mga maglalaro." && !cover) {
                        bgManager.setBackground("Covered.png");
                        cover = true;
                    }

                    if (currentDialogue == "Sa lahat ng maglalaro, sa kanya na pupunta ang atensyon ng lahat. Pano ba naman?\nSa tindig at galawa pa lang, ibang-iba na ang aura nya. Isama mo pa ang tangkad at\nkagwapuhan nya. May something sa kanya na parang nangaakit at hindi mo matanggal\nang iyong mga mata." && !covMaca) {
                        bgManager.setBackground("coverMaca.png");
                        covMaca = true;
                    }
                        ...
                        std::vector<Choice> choices = {
                            {"Pumayag", [&]() {
                                std::vector<DialogueEntry> choice1Dialogue = {
                                    ...
                                    {"I'm all ears.", "Maca"},
                                    {"Sino sya?", "???"},
                                    {"Mas gu-gustuhin nilang sila mismo ang magpakilala.", "Maca"},
                                    ...
                                    {"Sure?", "???"},
                                    {"Deretso kana kay toa na mag huwat na ang imong gihigugma (Dumiretso ka na,\nnaghihintay na yung nagmamahal sayo)", "Maca"},
                                    {"ITUTULOY...", "???"}
                            ...
                            {"Bumalik", [&]() {
                                std::vector<DialogueEntry> choice2Dialogue = {
                                    ...
                                    {"Di tayo dadaan sa portal malapit sa may palenque, marami masyadong maghahad-\nlang na makauwi ka.", "Maca"},
                        ...
                        std::vector<Choice> choices = {
                            ...
                            {"Wala na", [&]() {
                                std::vector<DialogueEntry> choice2Dialogue = {
                                    ...
                                    {"Wag na, kaya ko. Kailangan mo na umuwi sa bahay niyo. Alam kong may katanungan\nka pa pero kailangan mo ng tulog. Bibisitahin kita after para ma sagot ko ang tanong\nmo.", "Maca"},
                                    {"Thank you Maca.", "???"},
                                    //PICTURE OF MACA
                                    {"Walang anuman.”", "Maca"}
                    ...
                    if (!dialogue.isVisible() && !choiceBox.isVisible()) {
                        showDialogue = false;
                    }
                    ...
                    currentProgress.speaker = dialogue.getCurrentSpeaker();
                    currentProgress.dialogue = dialogue.getCurrentDialogue();
