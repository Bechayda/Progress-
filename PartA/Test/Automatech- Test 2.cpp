...
int main() {
    ...
    // Load sound effects
    if (!soundManager.loadSound("ECH", "ECH.ogg")) {
            ...
            while (window.isOpen()) {
                ...
                if (showDialogue) {
                    ...
                    if (currentDialogue == "Look Back.\nPan on the city of Biringan" && !chapter2Started) {
                        ...
                        ChapterManager::transitionToChapter(window, bodyFont, "Biringan");
                        bgManager.setBackground("House.png");

                        std::vector<DialogueEntry> chapter2Dialogues = {
                            {"Where... am I?", "???"},
                            {"This place... it wasn't here a moment ago.", "???"},
                            {"You have entered the City of Biringan.", "Mysterious Voice"}
                        };
